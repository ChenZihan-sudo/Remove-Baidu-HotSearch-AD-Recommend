...
function setButton() {
    ...
    if (pageLocation == "index") {
        ...
        if (loginstatus == false) {
            ...
            dpBtnF("dpBtnI", "padding:2px;margin:6px;margin-top:0px;", "ToOpen", "开启");
            document.getElementsByClassName("s-hotsearch-title")[0].appendChild(dpBtn);
            dpBtn.addEventListener('click', setHotSearch); //添加按钮监听
        } else {
            dpBtnF("dpBtnI", "padding: 2px;margin-right:170px;margin-top: -10px;", "ToOpen", "开启");
...
function setHotSearch() {
    ...
    if (document.getElementById("dpBtnI").value == "ToClose") {
        ...
        setDisplayOfHotSearch("display")
            //如果出现bug 更改为使用全局变量
    } else if (document.getElementById("dpBtnI").value == "ToOpen") {
...
function setDisplayOfHotSearch(dpyChange) {
    ...
    if (dpyChange == "display") {
        ...
        var setDisplay = ""
        if (loginstatus == true) {
            dpBtn.style = "padding: 2px; margin-left: 10px; margin-top:-5px";
...
console.log("启动脚本时的搜索词", getSearchWord())
...
console.log(findNodeByClass("nums", "new_nums"))