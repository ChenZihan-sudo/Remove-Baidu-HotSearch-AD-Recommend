...
(function() {
    ...
    function removeBaiduSeriesAD() {
        ...
        if (name == "baike.baidu.com") {
            ...
            while (document.getElementsByClassName("ad_bottom_view").length > 0) {
                for (var c = 0; c < document.getElementsByClassName("ad_bottom_view").length; c++) {
                    console.log(document.getElementsByClassName("ad_bottom_view"))
                    document.getElementsByClassName("ad_bottom_view")[c].parentNode.parentNode.parentNode.parentNode.parentNode.remove()
                }
            }
            //添加监听器
            seriesAgainRemoveWays(true, "ad_bottom_view", "thread_theme_7")
            seriesAgainRemoveWays(true, "ad_bottom_view", "thread_theme_5")

        } else if (name == "zhidao.baidu.com") {
            while (document.getElementsByClassName("wgt-ads").length > 0) {
                for (var b = 0; b < document.getElementsByClassName("wgt-ads").length; b++) {
                    document.getElementsByClassName("wgt-ads")[b].remove()
                }
            }
        } else if (name == "image.baidu.com") {
            if (path == "/search/index") {
                while (document.getElementsByClassName("fc-img").length > 0) {
                    document.getElementsByClassName("fc-img")[0].childNodes[0].remove()
                    for (var d = 0; d < document.getElementsByClassName("fc-img").length; d++) {
                        document.getElementsByClassName("fc-img")[0].childNodes[0].remove()
                    }
                }
            } else if (path == "/search/detail") {
                console.log(document.getElementsByClassName("text-link-ads"))
                while (document.getElementsByClassName("text-link-ads").length > 0) {
                    for (var a = 0; a < document.getElementsByClassName("text-link-ads").length; a++) {
                        document.getElementsByClassName("text-link-ads")[a].remove()
                    }
                }
            }
            ...
            if (location.path == "/search") {
                console.log("搜索界面");
                document.querySelector("#fengchaoad > div").remove()
            } else if (name.search("/view") != -1) {
                console.log("内容界面");
    ...
    function removeWays(selectedNode) {
        var node = document.querySelector(selectedNode)
        if (node != null) { node.remove() }
    }

    function seriesAgainRemoveWays(addListener, stopClassName, ListenerElementId) {
        if (addListener == true) {
            //document.getElementById(ListenerElementId).addEventListener("mousedown", setCheckInterval(stopClassName));
            document.body.addEventListener('DOMSubtreeModified', setCheckInterval(stopClassName));
        } else { setCheckInterval(stopClassName) }

        function setCheckInterval(stopClassName) {
            console.log("启动了")
            console.log("设置了点击或重复删除");
            // setTimeout(() => {
            //     var interval = setInterval(() => {
            //         console.log("延时启动");
            //         removeBaiduSeriesAD()
...
// DOMSubtreeModified
// document.body.addEventListener('DOMNodeRemovedFromDocument', function(){console.log("hello!")});